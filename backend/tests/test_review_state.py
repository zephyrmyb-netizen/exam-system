...
class TestReviewStateComputation:
    ...
    def test_interval_progression(self):
        ...
        for lvl in range(1, 6):
            current = models.UserQuestionReview(
                review_level=lvl - 1, consecutive_correct=lvl - 1,
                consecutive_wrong=0,
            )
    ...
    def test_wrong_answer_resets(self):
        now = datetime.now(timezone.utc)
        current = models.UserQuestionReview(
            review_level=3, consecutive_correct=3, consecutive_wrong=0,
        )
    ...
    def test_consecutive_wrong_increments(self):
        now = datetime.now(timezone.utc)
        current = models.UserQuestionReview(
            review_level=0, consecutive_correct=0, consecutive_wrong=3,
        )
        state = _compute_review_state(is_correct=False, current=current, now=now)
...
class TestReviewDB:
    ...
    def _setup(self, client, auth_headers):
        client.post(self.QUESTIONS_BATCH, json=[
            {"type": "single_choice", "question": "Q1: 1+1=?",
             "options": {"A": "1", "B": "2"}, "answer": "B",
             "subject": "数学", "chapter": "第一章"},
            {"type": "fill_blank", "question": "Q2: capital of China",
             "answer": "北京", "subject": "地理", "chapter": "第一章"},
            {"type": "true_false", "question": "Q3: Earth is round",
             "answer": "正确", "subject": "地理", "chapter": "第二章"},
    ...
    def test_due_endpoint(self, client, auth_headers, db_session):
        ...
        r = crud.get_user_question_review(db_session, user_id=1, question_id=qid)
        r.next_review_at = datetime.now(timezone.utc) - timedelta(minutes=1)
        db_session.commit()
