...
2. 已跟踪文本文件中的密钥泄露：API Key、SECRET_KEY、GitHub Token、Bearer Token、sk- 密钥。
3. .env.example 中的空值或 <your-api-key> 占位符被视为安全，不报错。
...
def main() -> int:
    ...
    # ── 检查 2：密钥泄露扫描 ──────────────────────────────────────────────
    p("=" * 60)
    p("\U0001f511 检查 2/2：已跟踪文件密钥泄露扫描")
    p("=" * 60)
