...
import { Search, RefreshCw, Eye, EyeOff, Trash2, ChevronLeft, ChevronRight } from "@lucide/vue";
...
function goPrevPage() {
  if (page.value > 1) { page.value--; fetchQuestions(); }
}

function goNextPage() {
  if (hasMore.value) { page.value++; fetchQuestions(); }
}
  ...
  <section class="stack">
    ...
    <article v-for="question in questions" :key="question.id" class="card question-card">
      ...
      <div class="meta-row">
        ...
        <span class="meta-type">{{ typeLabel(question.type) }}</span>
      </div>
      ...
      <!-- Options -->
      <ul v-if="formatOptions(question.options).length" class="option-list">
        <li v-for="option in formatOptions(question.options)" :key="option.key">
      ...
      <div v-if="isExpanded(question.id)" class="answer-panel">
        ...
        <div class="answer-line">
          ...
          <span class="answer-value">{{ question.analysis || "暂无解析" }}</span>
        </div>
      </div>

      <!-- Actions -->
      <div class="button-row">
        <button class="ghost-button" type="button" @click="toggleAnswer(question.id)">
          <EyeOff v-if="isExpanded(question.id)" :size="16" style="margin-right:4px" />
          <Eye v-else :size="16" style="margin-right:4px" />
          {{ isExpanded(question.id) ? "收起答案" : "查看答案" }}
        </button>
        <button class="danger-button" type="button" @click="deleteQuestion(question)">
